...
import pl.gregiem.warships.basic.IntPair;
...
public class Gameboard {
    ...
    public boolean tryAddTinyShip(int row, int column) {
        ...
        shipList[shipCount] = new Ship(new IntPair(row, column), 1, Orientation.horizontal);
        shipCount++;
        howManyLeft += 1;
        return true;
    }
    ...
    public void setRandom() {
        ...
        for (i = 0; i < Start.tinyShipsCount; i++)
            do {
                row = generator.nextInt(boardSize);
                column = generator.nextInt(boardSize);
            } while (!tryAddTinyShip(row, column));
